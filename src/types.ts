...
export type MarkdownEditorToolBarConfig = (MarkdownEditorToolBarName | '|' | MarkdownEditorToolBarItem)[];
...
export type MarkdownEditorStatus = false | ('autosave' | 'lines' | 'words' |'cursor' | {
  className: string;
  defaultValue: (el: HTMLElement) => void;
  onUpdate: (el: HTMLElement) => void;
})[];

export type MarkdownEditorShortcutsKey = 'toggleBlockquote' | 'toggleBold' | 'cleanBlock' | 'toggleHeadingSmaller' | 'toggleItalic' | 'drawLink' | 'toggleUnorderedList' | 'togglePreview' | 'toggleCodeBlock' | 'drawImage' | 'toggleOrderedList' | 'toggleHeadingBigger' | 'toggleSideBySide' | 'toggleFullScreen';
